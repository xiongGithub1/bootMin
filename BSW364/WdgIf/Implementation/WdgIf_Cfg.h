...
# define WDGIF_CFG_SW_MINOR_VERSION         (5u)
# define WDGIF_CFG_SW_PATCH_VERSION         (0u)

# define WDGIF_GENERATOR_MAJOR_VERSION      (2u)
# define WDGIF_GENERATOR_MINOR_VERSION      (3u)
...
# define WDGIF_MAX_SUPPORTED_CORES          (10u)
...
typedef struct
{
  Std_ReturnType (*Wdg_SetMode_AR) (WdgIf_ModeType Mode);          /*!< Pointer to a driver function
                                                                   *   Wdg_<platform>_SetMode()
                                                                   */ 
  void (*Wdg_SetTriggerCondition_AR) (uint16 Timeout);             /*!< Pointer to a driver function 
...
typedef struct
{
  ...
  WdgIf_StateCombinerSharedMemory *WdgIfStateCombinerSData;         /*!< Pointer to array of the shared memory fields
                                                                     ...
                                                                     */
  const WdgIf_StateCombinerSlaveTriggerPatternType * const * const WdgIfStateCombinerSlaveTriggerPattern;   /*!< Array of structures
                                                                                               * of State  Combiner
...
typedef struct
{
  ...
  const WdgIf_InterfaceFunctionsPerWdgDeviceType *WdgFunctionsPerDevice;  /*!< Array of structures functions per device
                                                                           *   ordered by device index
                                                                           */
# if (WDGIF_USE_STATECOMBINER == STD_ON)
  const WdgIf_StateCombinerConfigType *WdgIfStateCombinerConfig;          /*!< Array of structures of State Combiner
