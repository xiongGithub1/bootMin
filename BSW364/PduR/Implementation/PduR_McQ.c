...
PDUR_LOCAL_INLINE FUNC(void, PDUR_CODE) PduR_McQ_SerializeTxConfirmation(PduR_McQBufferArrayRamWriteIdxOfMcQBufferRamType writeIdx, PduR_RmSrcRomIdxOfRmDestRomType rmSrcRomIdx, PduR_MemIdxType memIdx)
{
  ...
  /* #20 Serialize rmSrcRomIdx */
  {
    ...
    for(; rmSrcRomIterator < sizeof(PduR_RmSrcRomIdxOfRmDestRomType); rmSrcRomIterator++)  /* FETA_PDUR_04 */
    {
...
FUNC(void, PDUR_CODE) PduR_McQ_MainFunction(PduR_DestApplicationManagerRomIterType destApplicationManagerRomIdx)
{
  ...
  for(srcApplicationRomIdx = PduR_GetSrcApplicationRomStartIdxOfDestApplicationManagerRom(destApplicationManagerRomIdx); srcApplicationRomIdx < PduR_GetSrcApplicationRomEndIdxOfDestApplicationManagerRom(destApplicationManagerRomIdx); srcApplicationRomIdx++)  /* FETA_PDUR_01 */
  {
    if(PduR_IsMcQBufferRomUsedOfSrcApplicationRom(srcApplicationRomIdx))
    {
      ...
      while(PduR_McQ_GetReadIdx(mcQBufferRomIdx, &readIdx, memIdx) == E_OK)     /* SBSW_PDUR_STACK_VARIABLE */ /* FETA_PDUR_02 */
      {
